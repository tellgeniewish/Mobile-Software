...
//    }
    ...
    fun addNewDrama(newFoodDto : DramaDto) {
        dramas.add(newFoodDto)
    }

    fun modifyDrama(pos: Int, modifyFoodDto : DramaDto) {
        dramas.set(pos, modifyFoodDto)
    }

    fun removeDrama(removeFoodDto : DramaDto) {
        val index = dramas.indexOf(removeFoodDto)
        dramas.removeAt(index)
    }

    fun getFoodByPos(pos : Int) = dramas.get(pos)
}